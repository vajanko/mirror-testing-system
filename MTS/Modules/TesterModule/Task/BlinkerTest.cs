...
namespace MTS.TesterModule
{
    sealed class BlinkerTest : RangeCurrentTest
    {
        ...
        public override void BeginExecute(TimeSpan time)
        {
            ...
            base.BeginExecute(time);    // this will switch on blinker
        }
        public override void Update(TimeSpan time)
        {
            ...
            {   // blinker is on
                ...
                {   // its time has elapsed
                    if (blinksCount == 0)   // no other blinks should be executed
                        EndExecute(time, TaskState.Completed);  // then finish this task
                    else
                    {   // more blinks should be executed
                        ControlChannel.SetValue(false);    // switch off light
                        blinksCount--;
                        blinkerOff = time;
                    }
                }   // otherwise - nothig just measure the current
            ...
            {   // blinker is off
                ...
                {   // its time has elapsed
                    ControlChannel.SetValue(true);      // switch on light
                    blinkerOn = time;
                }
            }   // do not measure current when blinker is off
        }
